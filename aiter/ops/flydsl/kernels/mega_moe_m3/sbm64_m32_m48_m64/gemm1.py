...
# fmt: on
    ...
    if const_expr(pipe_weights):
        ...
        for sp_i, state in range(0, loop_steps, 1, init=init):
            ...
            cached_a = None
            if const_expr(M_REPEAT in (2, 3, 4)):
...
def do_tile_short_or_full(m_tile, n_tile_base, expert, sched, a_gather, a_s2r,
    ...
    tvr_rsrc=None, tvr_row_bytes=0, wait_payload=None, work=None, mfma_short=None, a_gather_short=None, mfma_medium=None, a_gather_medium=None):
    ...
    if valid_rows > fx.Int32(48):
        ...
        acc = do_tile(m_tile, n_tile_base, expert, sched, a_gather_medium, a_s2r,
            b_loader, b_scale, a_scale, mfma_medium, epi, a_buf, a_scale_lds,
...
def build_fused_gemm1(*, x_tensor, w_rsrc, sw_rsrc, sx_rsrc,
    ...
    # Fixed M32/M48/M64 entry paths for the qualified SBM64/N256 workload.
    ...
    mfma_medium = MfmaScaleGU(m_repeat=3, num_acc_n=num_acc_n)
    ...
    a_gather_short = ATileLoader(row_bytes=model_dim, sort_block_m=32,
        k_step_bytes=a_k_step_bytes, total_threads=total_threads, swizzle=swizzle_a,
        x_tensor=x_tensor, async_copy=async_a_copy, tile_bytes=a_tile_bytes)
    a_gather_medium = ATileLoader(row_bytes=model_dim, sort_block_m=48,
        k_step_bytes=a_k_step_bytes, total_threads=total_threads, swizzle=swizzle_a,
        x_tensor=x_tensor, async_copy=async_a_copy, tile_bytes=a_tile_bytes)
    # fmt: off
    ...
    def do_scheduled_tile(flat, wait_payload=None):
        ...
        do_tile_short_or_full(m_tile, n_tile_base, expert, sched, a_gather,
            ...
            mfma_medium=mfma_medium, a_gather_medium=a_gather_medium)
        # fmt: on
