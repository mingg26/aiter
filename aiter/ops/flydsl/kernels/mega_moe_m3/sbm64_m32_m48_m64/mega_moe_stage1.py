...
def compile_mega_moe_stage1(
    *, model_dim: int, inter_dim: int, rank: int, experts_per_rank: int, fuse_npes: int, fuse_topk: int,
    ...
    if shared_packed:
        ...
        max_shared_claims = 3 * ceildiv(int(fuse_mtpr), sort_block_m) + num_cu * grid_mult
        max_routed_m = ceildiv(int(fuse_npes) * int(fuse_mtpr) * int(fuse_topk), sort_block_m) + experts_per_rank
        max_routed_claims = ceildiv(max_routed_m, BAND_M) * (3 * BAND_M) + num_cu * grid_mult
    ...
    leader_empty_drain = (shared_packed and int(fuse_mtpr) == 136
        and sort_block_m == 64 and tile_n == 256 and not schedule_audit)
    ...
    kernel_name = (
        ...
        + "_asb1"
        + "_svb1"
        + "_tf1_m324864_botharbd_epimid16_n256"
    ...
    def kernel(
        ...
        def _claim_routed(work_shard):
            ...
            if const_expr(small_xcd):
                # N256: home=(expert*24+n)%8; three local N slots per queue.
                # Keep the finite M-band ordering and canonical m*N+n indices.
                # Return canonical m*N+n indices to the unchanged GEMM.
                m_index = (local_work // fx.Int32(3 * BAND_M)) * fx.Int32(BAND_M) + local_work % fx.Int32(BAND_M)
                safe_m = (m_index < routed_m_tiles).select(m_index, fx.Int32(0))
                e_index = expert_of_flat(safe_m * fx.Int32(N_TILES))
                n_first = (work_shard + e_index * fx.Int32(0)) & fx.Int32(7)
                n_index = n_first + ((local_work // fx.Int32(BAND_M)) % fx.Int32(3)) * fx.Int32(8)
                ...
                ticket_count = ceildiv(routed_m_tiles, fx.Int32(BAND_M)) * fx.Int32(3 * BAND_M)
                    ...
                    if const_expr(shared_l13):
                        ...
                        if shared_active:
                            if const_expr(shared_xcd):
                                if const_expr(small_xcd):
                                    # N256: 24 N panels, three panels per queue.
                                    # Every CTA exhausts each queue exactly once.
                                    shared_small_m = (fuse_mtpr + sort_block_m - 1) // sort_block_m
                                    shared_limit = fx.Int32(3 * shared_small_m)
                                    if const_expr(shared_packed):
                                        shared_local = fx.Int32(comm_ops.atomic_add_agent(
