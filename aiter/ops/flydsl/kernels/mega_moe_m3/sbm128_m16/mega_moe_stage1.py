...
def compile_mega_moe_stage1(
    *, model_dim: int, inter_dim: int, rank: int, experts_per_rank: int, fuse_npes: int, fuse_topk: int,
    ...
    skip_empty_m16: bool = False,
):
    arch = str(get_rocm_arch() or "")
    if not arch.startswith("gfx95"):
        raise RuntimeError(f"MegaMoE v2 stage1 requires CDNA4 (gfx95x), got {arch or 'unknown'}")
    if skip_empty_m16:
        assert sort_block_m in (64, 128) and mfma_amajor and pipe_weights and not prefetch_a_operand and not unroll_a_pingpong
    ...
    if shared_l13:
        ...
        if int(fuse_mtpr) in SHARED_FUSED_MTPR_SMALL:
            ...
            want = SHARED_FUSED_S1_GEOMETRY["small_sbm128" if sort_block_m == 128 else "small"]
            ...
            assert sort_block_m in (32, 64, 128)  # EXPERIMENT: 128 for b256
    ...
    if xcd_schedule:
        ...
        if small_xcd:
            assert (tile_n, N_TILES) in ((512, 12), (256, 24))  # EXPERIMENT: add the large-regime pairing
            assert sort_block_m in (32, 64, 128)  # EXPERIMENT: 128 for b256
    ...
    if shared_packed:
        ...
        # ceil(N_TILES/8) panels per queue, matching the shared claim below.
        max_shared_claims = ((N_TILES + 7) // 8) * ceildiv(int(fuse_mtpr), sort_block_m) + num_cu * grid_mult
        max_routed_m = ceildiv(int(fuse_npes) * int(fuse_mtpr) * int(fuse_topk), sort_block_m) + experts_per_rank
        max_routed_claims = ceildiv(max_routed_m, BAND_M) * (((N_TILES + 7) // 8) * BAND_M) + num_cu * grid_mult
    ...
    leader_empty_drain = (shared_packed and int(fuse_mtpr) == 200
        and sort_block_m == 128 and tile_n == 256 and not schedule_audit)
    ...
    kernel_name = (
        ...
        f"{swiglu_suffix}"
        + ("_ems16k128sl64tb" if skip_empty_m16 else "")
        ...
        + ("_lp" if leader_empty_drain else "")
    ...
    def kernel(
        ...
        # All waves must consume the launch ticket before the work queue
        # or payload path reuses the same A-LDS storage.
        fx.barrier()
        ...
        # Leader-only phase: shared queues, routed queues, then done.
        ...
        def _claim_routed(work_shard):
            ...
            if const_expr(small_xcd):
                # Keep home=(m*N_TILES+n)%8 unchanged, but rotate through the
                # local N slots after each finite M band.
                # Return canonical m*N+n indices to the unchanged GEMM.
                #
                # Both constants below are functions of N_TILES, not literals:
                #   N_SLOTS = ceil(N_TILES/8) is how many 8-wide n groups a
                #     queue must visit to cover the N range (2 at N_TILES=12,
                #     3 at 24). n_index = n_first + 8*slot, so n_first fixes the
                #     residue mod 8 and slot picks the group.
                #   HOME_STEP = N_TILES % 8 is the per-m shift of the canonical
                #     home queue, since home = (m*N_TILES + n) % 8. At N_TILES=12
                #     it is 4, which reproduces the previous literal exactly
                #     (-4 == +4 mod 8); at 24 it is 0.
                # At N_TILES=12 this is byte-identical to the previous form.
                N_SLOTS = const_expr((N_TILES + 7) // 8)
                HOME_STEP = const_expr(N_TILES % 8)
                assert HOME_STEP in (0, 4), (
                    f"the home rotation below folds +/- together mod 8, which only "
                    f"holds for N_TILES%8 in (0,4); got {HOME_STEP}")
                m_index = (local_work // fx.Int32(N_SLOTS * BAND_M)) * fx.Int32(BAND_M) + local_work % fx.Int32(BAND_M)
                safe_m = (m_index < routed_m_tiles).select(m_index, fx.Int32(0))
                e_index = expert_of_flat(safe_m * fx.Int32(N_TILES))
                n_first = (work_shard + e_index * fx.Int32(HOME_STEP)) & fx.Int32(7)
                n_index = n_first + ((local_work // fx.Int32(BAND_M)) % fx.Int32(N_SLOTS)) * fx.Int32(8)
                ...
                ticket_count = ceildiv(routed_m_tiles, fx.Int32(BAND_M)) * fx.Int32(N_SLOTS * BAND_M)
        ...
        while consumer_active:
                    ...
                    if const_expr(shared_l13):
                        ...
                        if shared_active:
                            if const_expr(shared_xcd):
                                if const_expr(small_xcd):
                                    # Actual-batch M tiles; queue q owns the N panels
                                    # q, q+8, q+16, ... below N_TILES, i.e.
                                    # ceil((N_TILES - q) / 8) of them. Every CTA
                                    # exhausts each queue exactly once.
                                    # At N_TILES=12 this is the 2/2/2/2/1/1/1/1 split
                                    # the previous literal encoded; at 24 it is 3 each.
                                    shared_small_m = (fuse_mtpr + sort_block_m - 1) // sort_block_m
                                    shared_limit = ((fx.Int32(N_TILES + 7) - shared_work_shard) // fx.Int32(8)) * fx.Int32(shared_small_m)
                                    if const_expr(shared_packed):
                                        shared_local = fx.Int32(comm_ops.atomic_add_agent(
            ...
            if const_expr(leader_empty_drain):
                # Final exhausted ticket reaches every wave through four barriers.
...
def run_mega_moe_stage1(out, x, w, scale_x, scale_w, sorted_token_ids, expert_ids, num_valid_ids, out_scale,
    ...
    launch = compile_mega_moe_stage1(
        ...
        skip_empty_m16=bool(skip_empty_m16),
        swiglu_limit=swiglu_limit, swiglu_alpha=swiglu_alpha, swiglu_beta=swiglu_beta,
    )
    _run_compiled(
