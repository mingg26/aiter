...
# fmt: on
    ...
    if const_expr(mfma._skip_empty_m16):
        assert a_valid_rows is not None
        mfma.set_m_active(a_valid_rows)
    ...
    if const_expr(pipe_weights):
        ...
        if const_expr(async_a_copy):
            ...
            init += a_scale.load_step(a_scale_lds, fx.Int32(0), num_groups=NUM_A_SCALE)
        if const_expr(unroll_a_pingpong):
            ...
            def fixed_step(values, step: fx.Int32, phase):
                ...
                sa_next = a_scale.load_step(a_scale_lds, next_step, num_groups=NUM_A_SCALE)
        ...
        for sp_i, state in range(0, loop_steps, 1, init=init):
            ...
            if const_expr(async_a_copy):
                ...
                sa = a_scale.load_step(a_scale_lds, sp, num_groups=NUM_A_SCALE)

            cached_a = None
            if const_expr(M_REPEAT in (3, 4) and mfma._prefetch_a_operand):
                # M48/M64: consume the current LDS slot before the next DMA.
                # The K-end wait/barrier already made this current slot ready.
            ...
            if const_expr(async_a_copy):
                ...
                sa_next = None
                if const_expr(not ((M_REPEAT in (3, 4) and mfma._prefetch_a_operand)
                                   or (M_REPEAT == 8 and not mfma._prefetch_a_operand
                                       and not mfma._skip_empty_m16 and mfma_amajor
                                       and async_a_copy and return_acc_only))):
                    sa_next = a_scale.load_step(a_scale_lds, spn, num_groups=NUM_A_SCALE)
            else:
            ...
            if const_expr(M_REPEAT in (3, 4) and mfma._prefetch_a_operand
                          and not mfma._skip_empty_m16
                          and mfma_amajor and async_a_copy and return_acc_only):
                call_pipe = mfma.call_pipe_retire_b
            if const_expr(M_REPEAT == 8 and not mfma._prefetch_a_operand
                          and not mfma._skip_empty_m16
                          and mfma_amajor and async_a_copy and return_acc_only):
                def load_next_pack(ni, ks, _kn=spn):
                    return b_loader._load_pack(b_row, ni, _kn, ks)
                acc, b_next = mfma.call_pipe_ks_retire_b(
                    a_load, b_prev, acc, sa, sb, load_next_pack)
            else:
            ...
            if const_expr(async_a_copy):
                if const_expr((M_REPEAT in (3, 4) and mfma._prefetch_a_operand)
                              or (M_REPEAT == 8 and not mfma._prefetch_a_operand
                                  and not mfma._skip_empty_m16 and mfma_amajor
                                  and async_a_copy and return_acc_only)):
                    # Keep the completion wait after every current-step MFMA.
                    # This is a compiler scheduling boundary, not a new GPU sync.
                    rocdl.sched_barrier(0)
                    # Next-step scales have no dependency on current MFMA.
                    # Read them after compute, while next-B VMEM is in flight.
                    sa_next = a_scale.load_step(a_scale_lds, spn, num_groups=NUM_A_SCALE)
                wait_lds_barrier(
                    NUM_ACC_N * _PACK
                    + NUM_B_SCALE
        ...
        if const_expr(async_a_copy):
            ...
            sa = a_scale.load_step(a_scale_lds, last, num_groups=NUM_A_SCALE)
        ...
        if const_expr(unroll_a_pingpong):
            ...
            def streamed_step(values, step: fx.Int32, phase, has_next):
                ...
                sa = a_scale.load_step(a_scale_lds, step, num_groups=NUM_A_SCALE)
...
def do_tile_short_or_full(m_tile, n_tile_base, expert, sched, a_gather, a_s2r,
    ...
    if valid_rows <= fx.Int32(16):
        acc = do_tile(m_tile, n_tile_base, expert, sched, a_gather_short[0], a_s2r,
            b_loader, b_scale, a_scale, mfma_short[0], epi, a_buf, a_scale_lds,
            a_lds_i32, K_ITERS, 1, NUM_ACC_N, A_K_STEP_BYTES,
    ...
    elif valid_rows <= fx.Int32(32):
        acc = do_tile(m_tile, n_tile_base, expert, sched, a_gather_short[1], a_s2r,
            b_loader, b_scale, a_scale, mfma_short[1], epi, a_buf, a_scale_lds,
            a_lds_i32, K_ITERS, 2, NUM_ACC_N, A_K_STEP_BYTES,
            pipe_weights, mfma_amajor, async_a_copy, trb_rsrc,
            unroll_a_pingpong, split_a_lds, fp8_b_waitcnt, scalar_tile_row_base,
            tvr_rsrc=tvr_rsrc, tvr_row_bytes=tvr_row_bytes,
            wait_payload=wait_payload, work=work, return_acc_only=True)
    elif valid_rows <= fx.Int32(48):
        acc = do_tile(m_tile, n_tile_base, expert, sched, a_gather_short[2], a_s2r,
            b_loader, b_scale, a_scale, mfma_short[2], epi, a_buf, a_scale_lds,
            a_lds_i32, K_ITERS, 3, NUM_ACC_N, A_K_STEP_BYTES,
            pipe_weights, mfma_amajor, async_a_copy, trb_rsrc,
            unroll_a_pingpong, split_a_lds, fp8_b_waitcnt, scalar_tile_row_base,
            tvr_rsrc=tvr_rsrc, tvr_row_bytes=tvr_row_bytes,
            wait_payload=wait_payload, work=work, return_acc_only=True)
    elif valid_rows <= fx.Int32(64):
        acc = do_tile(m_tile, n_tile_base, expert, sched, a_gather_short[3], a_s2r,
            b_loader, b_scale, a_scale, mfma_short[3], epi, a_buf, a_scale_lds,
            a_lds_i32, K_ITERS, 4, NUM_ACC_N, A_K_STEP_BYTES,
            pipe_weights, mfma_amajor, async_a_copy, trb_rsrc,
            unroll_a_pingpong, split_a_lds, fp8_b_waitcnt, scalar_tile_row_base,
            tvr_rsrc=tvr_rsrc, tvr_row_bytes=tvr_row_bytes,
            wait_payload=wait_payload, work=work, return_acc_only=True)
    elif valid_rows <= fx.Int32(80):
        acc = do_tile(m_tile, n_tile_base, expert, sched, a_gather_short[4], a_s2r,
            b_loader, b_scale, a_scale, mfma_short[4], epi, a_buf, a_scale_lds,
            a_lds_i32, K_ITERS, 5, NUM_ACC_N, A_K_STEP_BYTES,
            pipe_weights, mfma_amajor, async_a_copy, trb_rsrc,
            unroll_a_pingpong, split_a_lds, fp8_b_waitcnt, scalar_tile_row_base,
            tvr_rsrc=tvr_rsrc, tvr_row_bytes=tvr_row_bytes,
            wait_payload=wait_payload, work=work, return_acc_only=True)
    elif valid_rows <= fx.Int32(96):
        acc = do_tile(m_tile, n_tile_base, expert, sched, a_gather_short[5], a_s2r,
            b_loader, b_scale, a_scale, mfma_short[5], epi, a_buf, a_scale_lds,
            a_lds_i32, K_ITERS, 6, NUM_ACC_N, A_K_STEP_BYTES,
            pipe_weights, mfma_amajor, async_a_copy, trb_rsrc,
            unroll_a_pingpong, split_a_lds, fp8_b_waitcnt, scalar_tile_row_base,
            tvr_rsrc=tvr_rsrc, tvr_row_bytes=tvr_row_bytes,
            wait_payload=wait_payload, work=work, return_acc_only=True)
    elif valid_rows <= fx.Int32(112):
        acc = do_tile(m_tile, n_tile_base, expert, sched, a_gather_short[6], a_s2r,
            b_loader, b_scale, a_scale, mfma_short[6], epi, a_buf, a_scale_lds,
            a_lds_i32, K_ITERS, 7, NUM_ACC_N, A_K_STEP_BYTES,
            pipe_weights, mfma_amajor, async_a_copy, trb_rsrc,
            unroll_a_pingpong, split_a_lds, fp8_b_waitcnt, scalar_tile_row_base,
            tvr_rsrc=tvr_rsrc, tvr_row_bytes=tvr_row_bytes,
            wait_payload=wait_payload, work=work, return_acc_only=True)
    else:
        acc = do_tile(m_tile, n_tile_base, expert, sched, a_gather_short[7], a_s2r,
            b_loader, b_scale, a_scale, mfma_short[7], epi, a_buf, a_scale_lds,
            a_lds_i32, K_ITERS, 8, NUM_ACC_N, A_K_STEP_BYTES,
            pipe_weights, mfma_amajor, async_a_copy, trb_rsrc,
            unroll_a_pingpong, split_a_lds, fp8_b_waitcnt, scalar_tile_row_base,
            tvr_rsrc=tvr_rsrc, tvr_row_bytes=tvr_row_bytes,
            wait_payload=wait_payload, work=work, return_acc_only=True)
    wait_lds_barrier()
    tile_row_base = _buffer_load(trb_rsrc, m_tile, fx.Int32)
    epi.store(acc, m_tile, tile_row_base, n_tile_base, skip_empty_upper_m64=True)
...
def build_fused_gemm1(*, x_tensor, w_rsrc, sw_rsrc, sx_rsrc,
    ...
    use_tile_resource, band_m=1, swiglu_limit=0.0, swiglu_alpha=1.702, swiglu_beta=1.0, packed_a_scale=False, unroll_a_pingpong=False, split_a_lds=False, fp8_b_waitcnt=False, prefetch_a_operand=False, scalar_tile_row_base=False, bf16_intermediate=None, a_tile_bytes=None, skip_empty_m16=False):
    # fmt: on
    ...
    a_gather_short = None
    if skip_empty_m16:
        assert m_repeat == 8 and tvr_rsrc is not None
        assert pipe_weights and mfma_amajor and async_a_copy
        assert not unroll_a_pingpong and not prefetch_a_operand
        # Select a fixed, fully unrolled M body once per tile. Physical LDS
        # strides and output layout remain 128 rows for every path.
        mfma_short = [MfmaScaleGU(m_repeat=mr, num_acc_n=num_acc_n,
                                  prefetch_a_operand=(mr in (3, 4)))
                      for mr in range(1, m_repeat + 1)]
        a_gather_short = [ATileLoader(row_bytes=model_dim, sort_block_m=mr * 16,
            k_step_bytes=a_k_step_bytes, total_threads=total_threads, swizzle=swizzle_a,
            x_tensor=x_tensor, async_copy=async_a_copy, tile_bytes=a_tile_bytes)
            for mr in range(1, m_repeat + 1)]
    # fmt: off
