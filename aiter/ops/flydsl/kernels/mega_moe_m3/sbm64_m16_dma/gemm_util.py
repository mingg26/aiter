...
class ATileLoader:
    ...
    def set_m_active(self, valid_rows):
        # All waves use the same true tile height; each DMA wave covers four rows.
        assert self._async_copy and self._total_threads == 512
        assert self._k_step_bytes == 256 and self._sort_block_m == 64
        self._m_active = valid_rows
        # Each wave has one group id; expose this invariant to scalar control flow.
        wave_group = (self._wave // fx.Int32(4)) * fx.Int32(16)
        self._dma_first_row = fx.Int32(rocdl.readfirstlane(T.i32, wave_group.ir_value()))
    ...
    def prefetch_to_lds(self, k_step_byte_off, lds_dst, base_i32=0):
        ...
        for round_base in range_constexpr(
            ...
            if const_expr(self._m_active is None):
                fx.copy(self._dma_atom, src, dst)
            else:
                # Four complete waves cover one 16-row group. The predicate is
                # uniform within each wave; partially valid groups still copy
                # all 16 rows, with the resource bound zero-filling padding.
                first_row = fx.Int32(round_base // 16) + self._dma_first_row
                active = self._m_active > first_row
                atom = self._dma_atom
