...
def do_tile_short_or_full(m_tile, n_tile_base, expert, sched, a_gather, a_s2r,
    ...
    tvr_rsrc=None, tvr_row_bytes=0, wait_payload=None, work=None, mfma_short=None, a_gather_short=None, mfma_mid=None, a_gather_mid=None):
    ...
    if valid_rows <= fx.Int32(M_REPEAT * 8):
        ...
        acc = do_tile(m_tile, n_tile_base, expert, sched, a_gather_mid, a_s2r,
            b_loader, b_scale, a_scale, mfma_mid, epi, a_buf, a_scale_lds,
...
def build_fused_gemm1(*, x_tensor, w_rsrc, sw_rsrc, sx_rsrc,
    ...
    if skip_empty_m16:
        ...
        mfma_short = MfmaScaleGU(m_repeat=m_repeat // 2, num_acc_n=num_acc_n)
        # Physical ping/pong offsets stay SBM-sized. The short arm never reads
        # the upper half, so it need not issue DMA or zero-fill LDS for it.
        a_gather_short = ATileLoader(row_bytes=model_dim, sort_block_m=sort_block_m // 2,
            k_step_bytes=a_k_step_bytes, total_threads=total_threads, swizzle=swizzle_a,
            x_tensor=x_tensor, async_copy=async_a_copy, tile_bytes=a_tile_bytes)
        mfma_mid = MfmaScaleGU(m_repeat=6, num_acc_n=num_acc_n)
        a_gather_mid = ATileLoader(row_bytes=model_dim, sort_block_m=96,
            k_step_bytes=a_k_step_bytes, total_threads=total_threads, swizzle=swizzle_a,
            x_tensor=x_tensor, async_copy=async_a_copy, tile_bytes=a_tile_bytes)
    # fmt: off
    ...
    def do_scheduled_tile(flat, wait_payload=None):
        ...
        if const_expr(skip_empty_m16):
            do_tile_short_or_full(m_tile, n_tile_base, expert, sched, a_gather,
                ...
                mfma_mid=mfma_mid, a_gather_mid=a_gather_mid)
        else:
