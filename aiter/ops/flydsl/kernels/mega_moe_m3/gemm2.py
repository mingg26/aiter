...
def issue_a_load_lds_dt(
    ...
    rows_per_wave = BM // num_waves  # rows each wave loads (BM32: 8, BM64: 16)
...
def gemm2_compute_big(
    ...
    *,
    ...
    if SBM is None:
        SBM = BM
    # Only the math rows change; BM still controls indexing, scale layout,
    # A DMA and LDS slots. Both paths return the physical BM32 output shape.
    compute_m = BM if compute_m is None else compute_m
    assert compute_m == BM or (BM in (32, 64, 128) and compute_m % 16 == 0 and 16 <= compute_m < BM)
    assert num_waves in (4, 8) and BN % (num_waves * 16) == 0
    kMChunks = compute_m // 16  # logical math rows; physical BM stays unchanged
    kHalves = BK // 128  # 16x16x128 MFMA K-steps per K-tile
    tilesPerScaleChunk = 256 // BK  # K-tiles sharing one 256-K E8M0 word
    numAccN = (BN // num_waves) // 16  # 16-column MFMA subblocks per wave
    ...
    is_bm16 = compute_m < 32
    rg_off = 0
    kScaleSubBlocks = max(1, (kMChunks + 1) // 2)
    ...
    K_BYTES = (fx.Int32(INTER_MAX) if num_waves == 8 else K_rt) // fx.Int32(a_pack)  # A row stride bytes (runtime)
    kc_rt = (fx.Int32(INTER_MAX) if num_waves == 8 else K_rt) // fx.Int32(256)  # (K//32)//4//2
    ...
    KH4 = (fx.Int32(INTER_MAX) if num_waves == 8 else K_rt) // fx.Int32(4)  # i32 col stride: FP8 is one byte per element
    ...
    asc_per_mb = fx.Int32(max(1, BM // 32)) * kAS_per_chunk_dw * fx.Int32(4)
    asc_num = fx.Int64(i32_max_m_blocks) * fx.Int64(asc_per_mb)
    # For logical M16 in physical BM32, m_block_idx == m_row // 32.
    scale_chunk0 = m_block_idx if const_expr(BM < 32) else m_row // 32
    ...
    if const_expr(num_waves == 8):
        # One bounded descriptor for all physical tile scale chunks.
        # Each chunk keeps its original offset; only descriptor reuse changes.
        asc_base_dw = scale_chunk0 * kAS_per_chunk_dw
        asc_rsrc = buffer_ops.create_buffer_resource_from_addr(
            fx.Int64(arg_ascale) + fx.Int64(asc_base_dw) * fx.Int64(4),
            num_records_bytes=asc_num - fx.Int64(asc_base_dw) * fx.Int64(4))
    ...
    def load_a_scale_tile(kt):
        ...
        for sub in range_constexpr(kScaleSubBlocks):
            if const_expr(num_waves == 8):
                offset_dw = fx.Int32(sub) * kAS_per_chunk_dw + lane_div_16 * fx.Int32(16) + lane_mod_16 + chunk_kt * fx.Int32(64)
                out.append(fx.Int32(buffer_ops.buffer_load(asc_rsrc, offset_dw, vec_width=1, dtype=fx.Int32)))
            else:
                saf = fx.make_fragment_like(sc_frag_tmpl)
                fx.copy(sc_copy_atom, ascale_views[sub][lane_div_16, lane_mod_16, chunk_kt, None], saf)
    ...
    bq_base_dw = [
        rocdl.readfirstlane(
            T.i32,
            (e * N_OUT_rt + n_block_idx * BN + wave * (BN // num_waves) + j * 16) * KH4,
        )
        for j in range_constexpr(numAccN)
    ]

    mni_base = n_block_idx * (BN // 16 // 2) + (wave * (BN // num_waves)) // 32
    bscale_views = [
        scale_view(
            arg_bscale,
    ...
    def issue_b_load_into(bqf, bsf, kt_rt):
        ...
        for j in range_constexpr(numAccN):
            for half in range_constexpr(kHalves):
                ...
                load_mask = None
                if const_expr(has_pad):
                    col = n_block_idx * BN + wave * (BN // num_waves) + j * 16
    ...
    def mfma_cluster(bqf, bsf, sa, kt_rt):
        ...
        for J in range_constexpr(numAccN):
            mni, in_b = J // 2, J % 2
            sb = sb_words[mni]
            if const_expr(BN // num_waves == 16):
                # Adjacent waves share one 32-column scale word. Shift its
                # odd-column bytes to opselB=0/2 for the odd wave.
                sb = fx.Int32(sb).shrui((wave % fx.Int32(2)) * fx.Int32(8))
            for sub in range_constexpr(kScaleSubBlocks):
                mma_one_j(
                    J, in_b, sa[sub], sb, bqf, a_frags, c_frags,
                    mma_atoms, i0=2 * sub,
                    single_rg=(2 * sub + 1 >= kMChunks),
                    rg_off=0, k_halves=kHalves,
                )
    ...
    if const_expr(BM == 64 and BN == 256 and num_waves == 4):
    ...
    if const_expr(compute_m < BM):
        accm_vecs += [[zero4 for _ in range_constexpr(numAccN)]
                      for _ in range_constexpr((BM - compute_m) // 16)]
