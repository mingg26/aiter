...
class MfmaScaleGU:
    ...
    def call_pipe_retire_ks(self, a_load, b_prev, acc, sa, sb, load_next_ks):
        """Consume one K128 half before loading that half of the next K256 step."""
        assert self._m_repeat in (1, 2) and self._num_acc_n == 2
        b_next = [[None] * _PACK for _ in range_constexpr(self._num_acc_n)]
        for ks in range_constexpr(_PACK):
            for mi in range_constexpr(self._m_repeat):
                a_op = a_load(mi, ks)
                for ni in range_constexpr(self._num_acc_n):
                    idx = self.idx(mi, ni)
                    acc[idx] = self._mfma(a_op, b_prev[ni][ks], acc[idx],
                        sa[mi // _PACK], sb[ni // _PACK], ks, mi % _PACK, ni % _PACK)
            # All consumers of this B half have retired; its registers can be reused.
            rocdl.sched_barrier(0)
            for ni in range_constexpr(self._num_acc_n):
                b_next[ni][ks] = load_next_ks(ni, ks)
            rocdl.sched_barrier(0)
        return acc, b_next
...
class SiluQuantEpilogue:
    ...
    def store(self, acc, tile_i32, tile_row_base_i32, n_tile_base_i32):
        ...
        def combine_store_tail():
            if tile_valid_rows > fx.Int32(16):
                store_rows(1, 2, self._combine(acc, first=1, last=2))
        # Retire the optional accumulator group before the common lower rows,
        # following SBM128's shared-epilogue ordering.
        combine_store_tail()

        store_rows(0, 1, self._combine(acc, first=0, last=1))
        gpu.barrier()
        ...
        assert (self._sort_block_m, self._num_waves, rows_per_iter, m_reps) == (32, 8, 16, 2)
