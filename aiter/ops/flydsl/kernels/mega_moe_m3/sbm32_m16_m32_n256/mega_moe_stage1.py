...
def compile_mega_moe_stage1(
    *, model_dim: int, inter_dim: int, rank: int, experts_per_rank: int, fuse_npes: int, fuse_topk: int,
    ...
    N_SLOTS = (N_TILES + 7) // 8
    n256_full = (tile_n == 256 and sort_block_m == 32 and int(fuse_mtpr) == 72
        and (model_dim, inter_dim, int(fuse_npes), experts_per_rank, int(fuse_topk)) == (6144, 3072, 8, 16, 4)
        and shared_l13 and shared_xcd and preplanned and xcd_schedule
        and num_waves == 8 and not fixed_slot_dispatch)
    ...
    if xcd_schedule:
        ...
        if small_xcd:
            assert (tile_n, N_TILES) == (512, 12) or (n256_full and N_TILES == 24)
            assert sort_block_m in (32, 64)
    ...
    if shared_packed:
        ...
        max_shared_claims = N_SLOTS * ceildiv(int(fuse_mtpr), sort_block_m) + num_cu * grid_mult
        max_routed_m = ceildiv(int(fuse_npes) * int(fuse_mtpr) * int(fuse_topk), sort_block_m) + experts_per_rank
        max_routed_claims = ceildiv(max_routed_m, BAND_M) * (N_SLOTS * BAND_M) + num_cu * grid_mult
    ...
    kernel_name = (
        ...
        + "_svb1"
        + "_scratchfix1"
        + ("_tb1_n256s" if n256_full else "_tb1_salate1_brni1")
    ...
    def kernel(
        ...
        def _claim_routed(work_shard):
            ...
            if const_expr(small_xcd):
                # Preserve home=(expert*N_TILES+n)%8 and rotate each local
                # N slot after one finite M band (2 slots at N512, 3 at N256).
