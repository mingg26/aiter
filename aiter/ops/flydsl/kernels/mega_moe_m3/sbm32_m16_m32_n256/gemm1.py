...
def build_fused_gemm1(*, x_tensor, w_rsrc, sw_rsrc, sx_rsrc,
    ...
    # Fixed M16/M32 entry paths, adapted from retained SBM64 common-epilogue implementation.
    assert (sort_block_m, tile_n, m_repeat, num_acc_n) == (32, 256, 2, 2)
    ...
    mfma_short = MfmaScaleGU(m_repeat=1, num_acc_n=num_acc_n)
    # Retain physical SBM32 ping/pong offsets and common M32 epilogue.
    a_gather_short = ATileLoader(row_bytes=model_dim, sort_block_m=16,
        k_step_bytes=a_k_step_bytes, total_threads=total_threads, swizzle=swizzle_a,
        x_tensor=x_tensor, async_copy=async_a_copy, tile_bytes=a_tile_bytes)
    # fmt: off
